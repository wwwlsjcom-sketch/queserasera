...
2. 使用 `template-router.md` 和 `template-matrix.yaml` 选择报告模板：classic-consulting、board-brief、market-intelligence、investment-memo、risk-review、product-strategy。
...
6. 为封面生成或构造一张与主题、行业、报告类型匹配的背景图。优先使用生成式 raster image，并嵌入为 base64 data URI；若不能生成图片，则用自包含 CSS/SVG 构造主题背景。标题排版必须根据图像留白和视觉重心调整。
...
- 必须使用 Exhibit 逻辑：编号、结论式标题、清晰轴线/图例、克制蓝灰配色、来源/假设说明。
